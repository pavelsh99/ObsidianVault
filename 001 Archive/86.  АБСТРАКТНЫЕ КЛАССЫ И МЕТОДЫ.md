...
Абстрактные свойства это по сути методы (`get` `set`), которые маскируются под поля.

То есть если это метод, то он может быть абстрактным, виртуальным, его можно переопределить в наследниках.

Добавим в наш класс `Weapon` абстрактное свойство `Damage` 

И поменяем метод `GetInfo()`, чтобы можно было смотреть урон при выводе метода
...
И мы убрали `set` (сеттер) чтобы нельзя было менять урон на уровне экземпляра.

Теперь компилятор требует переопределить свойства на уровне наследников абстрактного класса, то есть в классах `LaserGun`, `Cannon`, `Gun`.
...
```csharp
    class Cannon : Weapon
    {
        public override int Damage => 10; // ЛЯМБДА
        public override void Fire()
        {
            Console.WriteLine("Дыдыщь");
        }
    ...
    class LaserGun : Weapon
    {
        public override int Damage { get { return 15; } } // Классика
    ...
    class Gun : Weapon
    {
        public override int Damage { get { return 10; } }// Классика
        public override void Fire()
        {
            Console.WriteLine("Бум");
        }
